...
import CInputNumber from '../../../../../app/components/inputnumber';
import { yupResolver } from '@hookform/resolvers/yup';
const { Option } = Select;

const InputNumberPlugin = React.memo((props) => {
  const {
    state,
    readOnly,
    onChange,
  ...
  const getValidateSetting = function () {
    if (handleForm) {
      let fieldValue = yup.number().nullable(true);
      if(item.required){
...
export default withLocalize(InputNumberPlugin);
